...
h1 = figure(fig_idx);
hold on;
p1 = plot(err_opg, '-.', 'LineWidth', 1.5,'Color', 'r');
hold on;
p2 = plot(err_odpg, '-.', 'LineWidth', 1.5,'Color', 'b');
hold on;
p3 = plot(err_opadmm, '-', 'LineWidth', 1.5,'Color', 'm');

grid on;
ax = gca;
ax.GridLineStyle = '--';
ax.GridAlpha = 0.75;
legend([p1 p2 p3], {'online PG','online DPG','online P-ADMM'},'Location','southwest','FontSize',14);
xlabel('$k$','Interpreter','Latex');
xticks(ticks);
ylim([1e-2,1e+1]);
%ylim(y_range);
ylabel('$\|\mathbf{w}^{(k)} - \hat{\mathbf{w}}\|_{2}$','Interpreter','Latex','FontSize',14);
set(gca, 'YScale','log');